// 由于badjs-report是通过改写window.onerror来进行错误捕获和上报，
// 但当不同源的js文件抛出异常时，window.onerror无法获取具体的出错信息。
/*
    由于badjs-report是通过改写window.onerror来进行错误捕获和上报,
    但当不同源的js文件抛出异常时，window.onerror无法获取具体的出错信息。
    而badjs的解决方案是使用try-catch将可能出错的函数包裹起来，在catch里面
    就把错误处理掉。
    详见：https://github.com/BetterJS/badjs-report/issues/3
*/
        ...
        return function() {
            ...
            try {
                ...
                if (!timeoutkey) {
                    ...
                    // 先将window.onerror函数置为空函数，避免重复处理error事件
                    global.onerror = function() { };
                    // catch函数处理完error后再还原window.onerror
